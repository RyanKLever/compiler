...
; **********************************************************************************************
FIB 			
				; -- INSIDE FUNCTION PREP ------------------------------------------------------
				; Compute space needed for the rest of activation record (local, temp)
				; One 3 temporary variables needed
				CMP R0, R0				
				ADI R0, 3				; no local, three temp vars needed  
				CMP R1, R1
				ADI R1, 4				; size of stack word
				MUL R0, R1				; ************** FrameSize => R0
				...
				; Test for Overflow
				MOV R1, SP
				SUB R1, R0				; SP - frameSize => R1
				CMP R1, SL
				BLT R1, overflow 		; if (SP - framesize) < SL, OVERFLOW
